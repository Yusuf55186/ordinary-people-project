...
import {
  resolveYusufDeskPose,
  type YusufDeskPoseId,
} from "../production/poses/yusufDeskPoseRegistry";
import { yusufDeskIntegration } from "../production/characters/yusufDeskIntegration";

type YusufDeskShotProps = {
  poseId?: YusufDeskPoseId;
  children?:ReactNode;
  hesitation?:number;
  retreat?:number;
...
export const YusufDeskShot = ({poseId, phoneHoldProgress,phoneReachProgress,heldPhone,leftHandPose,rightArmPose,rightHandPose,yusufMode,typingToRestProgress,children,hesitation = 0 , retreat = 0,beanbagForeground = false,foregroundChildren = false,preformanceOffset = {}}: YusufDeskShotProps,) => {
  const resolvedPose = resolveYusufDeskPose(poseId);
  const resolvedYusufMode = yusufMode ?? resolvedPose.yusufMode;
  const resolvedTypingToRestProgress = typingToRestProgress ?? resolvedPose.typingToRestProgress;
  const resolvedPhoneReachProgress = phoneReachProgress ?? resolvedPose.phoneReachProgress;
  const resolvedPhoneHoldProgress = phoneHoldProgress ?? resolvedPose.phoneHoldProgress;
  const resolvedRightArmPose = rightArmPose ?? resolvedPose.rightArmPose;
  const resolvedRightHandPose = rightHandPose ?? resolvedPose.rightHandPose;
...
bodyY: seatedIdle.bodyY
  ...
  const typingOpacity =
  resolvedYusufMode === "typing" ? 1 - resolvedTypingToRestProgress : 0;

const modularOpacity =
  resolvedYusufMode === "typing" ? resolvedTypingToRestProgress : 1;
  ...
  <SceneMaster {...yusufDeskIntegration.typing}>
    <YusufBackTypingPose />
  </SceneMaster>
  ...
  <SceneMaster {...yusufDeskIntegration.modular}>
    <div
      style={{
        position: "absolute",
        top: 0,
    ...
    <div style={{ position: "relative", zIndex: 2 }}>
      <YusufBackCharacter
        {...finalPose}
        rightArmPose={resolvedRightArmPose === "rest" ? undefined : resolvedRightArmPose}
        rightHandPose={resolvedRightHandPose}
        leftHandPose={leftHandPose}
        lowerBodyPose="deskSeated"
        phoneReachProgress={resolvedPhoneReachProgress}
        phoneHoldProgress={resolvedPhoneHoldProgress}
